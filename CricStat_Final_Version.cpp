...
string nameArray[TOTAL_PLAYERS];
int MatchesArray[TOTAL_PLAYERS];
float RunsArray[TOTAL_PLAYERS];
float AverageArray[TOTAL_PLAYERS];
float CenturiesArray[TOTAL_PLAYERS];
float WicketsArray[TOTAL_PLAYERS];
float EconomyArray[TOTAL_PLAYERS];
...
void loadFromFile()
{
    ...
    while (!file.eof())
    {
        getline(file, nameArray[index]);
        if (nameArray[index] == "")
        {
            break;
        }

        file >> MatchesArray[index];
        file >> RunsArray[index];
        file >> AverageArray[index];
        file >> CenturiesArray[index];
        file >> WicketsArray[index];
        file >> EconomyArray[index];
        file.ignore();
        index++;
    }
    file.close();
...
void showICCRanking()
{
    ...
    while (i < index)
    {
        ...
        while (j < index)
        {
            if (sortedRanking[j] > sortedRanking[maxIndex])
            {
                maxIndex = j;
            }
            j++;
        ...
        sortedName[i] = sortedName[maxIndex];
        sortedName[maxIndex] = tempName;

        int tempMatches = sortedMatches[i];
        sortedMatches[i] = sortedMatches[maxIndex];
        sortedMatches[maxIndex] = tempMatches;

        float tempRuns = sortedRuns[i];
        sortedRuns[i] = sortedRuns[maxIndex];
        sortedRuns[maxIndex] = tempRuns;

        float tempAverage = sortedAverage[i];
        sortedAverage[i] = sortedAverage[maxIndex];
        sortedAverage[maxIndex] = tempAverage;

        float tempCentury = sortedCenturies[i];
        sortedCenturies[i] = sortedCenturies[maxIndex];
        sortedCenturies[maxIndex] = tempCentury;

        float tempWickets = sortedWickets[i];
        sortedWickets[i] = sortedWickets[maxIndex];
        sortedWickets[maxIndex] = tempWickets;

        float tempEconomy = sortedEconomy[i];
        sortedEconomy[i] = sortedEconomy[maxIndex];
        sortedEconomy[maxIndex] = tempEconomy;

        float tempRanking = sortedRanking[i];
        sortedRanking[i] = sortedRanking[maxIndex];
        sortedRanking[maxIndex] = tempRanking;
        i++;
    }
...
void deletePlayer()
{
    ...
    if (found == true)
    {
        ...
        cout << "Record of " << name << " deleted and saved to file!" << endl;
    }
    else
    {
        cout << "Record not found" << endl;
...
void addPlayer()
{
    ...
    // add new players data into recent index
    ...
    cout << "Player added successfully and saved to file!" << endl;
}
